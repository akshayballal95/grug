#!/usr/bin/env python3
"""Render the README benchmark charts from benchmarks/kimi-k2.5-full/results.json.
...
RESULTS = ROOT / "benchmarks" / "kimi-k2.5-full" / "results.json"
...
ENTITIES = {
    "rules": ("grug rules", 0),
    "mmbert-small": ("grug classifier", 1),
...
def quality_chart(rows: dict[str, dict], theme: dict) -> str:
    ...
    offsets = {"rules": (11, -16), "mmbert-small": (11, -16), "lingua2": (11, 24)}
    ...
    return (
        ...
        f'classifier keeps F1 0.71 at 37%; LLMLingua-2 sits lowest.">\n{body}\n</svg>\n'
    )
...
def negation_chart(rows: dict[str, dict], theme: dict) -> str:
    ...
    x_max = 50.0  # percent
    order = ["lingua2", "mmbert-small", "rules"]  # most losses first
    ...
    return (
        ...
        f'LLMLingua-2 drops 43%, the grug classifier 2%, grug rules none.">\n{body}\n</svg>\n'
    )
